...
namespace Movies.Client.Services
{
    public class StreamService : IIntegrationService
    {
        ...
        public async Task Run()
        {
            ...
            //await GetExtensionPosterWithStreamAndCompletionMode();
        ...
        private async Task GetPosterWithStreamAndCompletionMode()
        {
            ...
            //Default behavior : The operation should complete after reading entire response including content.
            //We want to start working on response content as soon as possible, in other words, when it's still coming in. To allow that
            // we need to use HttpCompletionOption.ResponseHeadersRead.
        ...
        private async Task GetExtensionPosterWithStreamAndCompletionMode()
        {
            var request = new HttpRequestMessage(
                HttpMethod.Get,
                $"api/movies/d8663e5e-7494-4f81-8739-6e0de1bea7ee/posters/{Guid.NewGuid()}");
        ...
        private async Task TestGetPosterWithStreamAndCompletionMode()
        {
            ...
            Console.WriteLine($"Elapsed milliseconds with stream and completion mode:" +
                              $"{stopWatch.ElapsedMilliseconds}, " +
                              $"averaging {stopWatch.ElapsedMilliseconds / 200} milliseconds/request");
        }
        ...
        private async Task PostPosterWithoutStream()
        {
            ...
            var content = await response.Content.ReadAsStringAsync();
            var createdPoster = JsonConvert.DeserializeObject<Poster>(content);
